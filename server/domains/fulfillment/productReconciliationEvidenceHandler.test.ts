...
import { createFulfillmentProductReconciliationEvidenceHandler } from "./productReconciliationEvidenceHandler.js";

describe("fulfillment product reconciliation evidence handler", () => {
  it("reads reconciliation evidence through the fulfillment port with the parsed filter", async () => {
    const port = createPort(response());
    const res = createResponse();

    await createHandler({ port })(request("GET", { statusFilter: "resolved" }), res);

    expect(port.getProductReconciliationEvidence).toHaveBeenCalledWith({ statusFilter: "resolved" });
  ...
  it("defaults the status filter to open when omitted", async () => {
    ...
    expect(port.getProductReconciliationEvidence).toHaveBeenCalledWith({ statusFilter: "open" });
  });
  ...
  it("maps authorization failures to the shared BFF envelope", async () => {
    const res = createResponse();
    await createFulfillmentProductReconciliationEvidenceHandler({
...
function createHandler({
  ...
  port: FulfillmentProductReconciliationEvidencePort;
  authorized?: boolean;
}) {
...
function createPort(result: unknown): FulfillmentProductReconciliationEvidencePort {
  return {
    getProductReconciliationEvidence: vi.fn().mockImplementation(async () => {
    ...
    items: [
      {
        ...
        sku: "GHOST-SKU",
        ean: "5902000000002",
        conflictKind: "unknown_sku" as const,
        providerQuantity: 1,
      ...
      },
      {
        ...
        ean: null,
        conflictKind: "kind_quantity_conflict" as const,
        providerQuantity: 6,
        status: "open" as const,
