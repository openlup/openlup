...
export function createSupabaseSubscriptionAutoResumePort(
  client: RpcClient,
): SubscriptionAutoResumePort {
  return {
    async run(limit: number): Promise<AutoResumeJobResult> {
...
function resultFromError(error: RpcError): AutoResumeJobResult {
  ...
  return {
    ok: false,
    skipped: false,
    reason: safeMessage(error.message ?? "subscription_auto_resume_rpc_failed"),
    scanned: 0,
    resumed: 0,
    skippedRows: 0,
    failed: 1,
...
function toResult(data: unknown): AutoResumeJobResult {
  const row = Array.isArray(data) ? data[0] : data;
  const value = row && typeof row === "object" ? row as Record<string, unknown> : {};
  return {
    ok: value.ok !== false,
    scanned: numberValue(value.scanned),
    resumed: numberValue(value.resumed),
