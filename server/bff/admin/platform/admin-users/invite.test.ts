...
describe("admin platform invite user BFF composition", () => {
  it("keeps Supabase function calls behind the admin-user invite port", () => {
    const source = readFileSync(
      join(process.cwd(), "server/bff/admin/platform/admin-users/invite.ts"),
      "utf8",
    );
    ...
    expect(source).toContain("createSupabaseAdminUserInvitePort({ accessToken })");
  });
});
