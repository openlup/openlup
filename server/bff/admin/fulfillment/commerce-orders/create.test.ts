...
import routeHandler from "./create.js";
import listRouteHandler from "../commerce-orders.js";
import detailRouteHandler from "./detail.js";
...
describe("admin fulfillment create route boundary", () => {
  it("uses the fulfillment gateway instead of direct service-role port construction", () => {
    const source = read("server/bff/admin/fulfillment/commerce-orders/create.ts");
  ...
  it.each([
    ["list", listRouteHandler, {}],
    ["detail", detailRouteHandler, { orderId: "42222222-2222-4222-8222-222222222221" }],
  ] as const)("executes the %s read route fail-closed without managed config", async (_name, handler, query) => {
    const res = response();
    await handler(
      { method: "GET", body: {}, query, headers: {} } as unknown as VercelRequest,
      res,
    );
    expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ ok: false }));
  });
