...
import handler from "./email-sends.js";
...
const mocks = vi.hoisted(() => ({
  ...
  createCommunicationsAdminEmailSendsHandler: vi.fn(),
}));
...
vi.mock("../../../domains/communications/adminEmailSendsHandler.js", () => ({
  createCommunicationsAdminEmailSendsHandler: mocks.createCommunicationsAdminEmailSendsHandler,
}));
...
describe("communications email sends admin BFF route", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("authorizes with the user-scoped client but reads the ledger through the communications gateway", async () => {
    ...
    mocks.createCommunicationsAdminEmailSendsHandler.mockImplementation((deps) => async (
      req: VercelRequest,
      res: VercelResponse,
    ) => {
