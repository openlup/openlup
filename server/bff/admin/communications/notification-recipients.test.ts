...
describe("communications notification recipients admin BFF route", () => {
  ...
  it("fails closed for an invalid bearer", async () => {
    const res = response();
    await handler({ method: "GET", headers: { authorization: "Bearer invalid" } } as never, res as never);
