...
describe("admin commerce promotion update BFF route", () => {
  ...
  it("executes the list route and fails closed when Supabase env is absent", async () => {
    const res = response();
    await listHandler(request(), res);
    expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ ok: false }));
  });
});
