...
describe("/api/bff/customers/billing-profiles route", () => {
  ...
  it("fails closed before constructing Supabase clients when disabled", async () => {
    const { default: handler } = await import("./billing-profiles.js");
  ...
  it("executes invoice subroutes and fails closed before clients when disabled", async () => {
    const [{ default: correction }, { default: download }] = await Promise.all([
      import("./invoices/correction-request.js"),
      import("./invoices/download.js"),
    ]);

    for (const handler of [correction, download]) {
      const res = createResponse();
      await handler(request("GET"), res);
      expect(res.status).toHaveBeenCalledWith(503);
    }
