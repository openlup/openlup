...
describe("GET /api/bff/customers/addresses route", () => {
  ...
  it("fails closed before creating a client when the feature flags are off", async () => {
    const { default: handler } = await import("./addresses.js");
  ...
  it("returns an error envelope when enabled without Supabase env", async () => {
    process.env.COMMERCE_V2_W12_CUSTOMER_AUTH_UI = "true";
    process.env.COMMERCE_CUSTOMER_ADDRESSES_ENABLED = "true";
    const { default: handler } = await import("./addresses.js");
    const res = createResponse();
